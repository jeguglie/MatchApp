...
async function signup(req, res) {
    ...
    try {
        ...
        const message = {
            ...
            text: `Hello !\nHere is the link to confirm your account ${process.env.SERVER_LOCALHOST !== JSON.stringify(1) ? 'https://matchappli.herokuapp.com/login/' : 'http://localhost:3000/login/'}`+hashtoken,
        };
...
async function login(req, res) {
    ...
    try {
        ...
        if (typeof response !== 'undefined' && typeof response.rows !== 'undefined' && response.rows.length) {
            ...
            if (process.env.SERVER_LOCALHOST === JSON.stringify(1))
...
async function userforgot(req, res){
    ...
    if (typeof response !== 'undefined' && typeof response.rows !== 'undefined' && response.rows.length){
        ...
        const message = {
            ...
            text: `Hello !\nHere is the link to reset your password ${process.env.SERVER_LOCALHOST !== JSON.stringify(1) ? 'https://matchappli.herokuapp.com/forgotpassword/' : 'http://localhost:3000/forgotpassword/'}`+hashtoken,
        };
