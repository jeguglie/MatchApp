...
export default class FileUpload extends React.Component {
    ...
    imagesFilesUpload = async (file) => {
        ...
        if (fileSize >= 4096)
            ...
            const {data} = await axios.post(process.env.NODE_ENV === "production" ? "https://matchappli.heroku.com/api/user-profile" : "http://localhost:5000/api/user-profile", formData, {withCredentials: 'true'});
            if (data.warnings)
