...
export default class FileUpload extends React.Component {
    ...
    imagesFilesUpload = async (file) => {
        ...
        if (fileSize >= 4096)
            ...
            const {data} = await axios.post(process.env.REACT_APP_LOCALHOST !== "true" ? "https://matchappli.herokuapp.com/api/user-profile" : "http://localhost:5000/api/user-profile", formData, {withCredentials: 'true'});
            if (data.warnings)
